...
class DoubleQLearningAgent:
    def __init__(self, grid, learning_rate=0.1, discount_factor=0.9, exploration_rate=1.0, min_exploration_rate=0.01, exploration_decay=0.995):
        ...
        self.q_table_1 = np.zeros((self.rows, self.cols, 4))
        self.q_table_2 = np.zeros((self.rows, self.cols, 4))
    ...
    def choose_action(self, state):
        if random.uniform(0, 1) < self.epsilon:
            ...
            q_values_1 = self.q_table_1[state[0], state[1]]
            q_values_2 = self.q_table_2[state[0], state[1]]
            return np.argmax(q_values_1 + q_values_2)  # Exploitation
        ...
        for episode in range(max_episodes):
            ...
            while not done and steps < 200:
                ...
                if random.uniform(0, 1) < 0.5:
                    best_next_action = np.argmax(self.q_table_1[next_state[0], next_state[1]])
                    td_target = reward + self.gamma * self.q_table_2[next_state[0], next_state[1], best_next_action]
                    td_error = td_target - self.q_table_1[current_state[0], current_state[1], action]
                    self.q_table_1[current_state[0], current_state[1], action] += self.lr * td_error
                else:
                    best_next_action = np.argmax(self.q_table_2[next_state[0], next_state[1]])
                    td_target = reward + self.gamma * self.q_table_1[next_state[0], next_state[1], best_next_action]
                    td_error = td_target - self.q_table_2[current_state[0], current_state[1], action]
                    self.q_table_2[current_state[0], current_state[1], action] += self.lr * td_error
    ...
    def print_q_table(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                print(f"État ({r},{c}):")
                for a in range(4):
                    print(f"  {self.action_names[a]}: {self.q_table_1[r,c,a]:.2f}")
            print()
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                print(f"État ({r},{c}):")
                for a in range(4):
                    print(f"  {self.action_names[a]}: {self.q_table_2[r,c,a]:.2f}")
            print()

class DoubleQLearningAgentClean:
    def __init__(self, grid, learning_rate=0.1, discount_factor=0.9, exploration_rate=1.0, min_exploration_rate=0.01, exploration_decay=0.995):
        self.grid = grid
        self.rows, self.cols = grid.shape
        self.q_table_1 = np.zeros((self.rows, self.cols, 4))
    ...
    def choose_action(self, state):
        if random.uniform(0, 1) < self.epsilon:
            ...
            q_values_1 = self.q_table_1[state[0], state[1]]
            q_values_2 = self.q_table_2[state[0], state[1]]
            return np.argmax(q_values_1 + q_values_2)
...
class DoubleQLearningAgentStations:
    def __init__(self, grid, learning_rate=0.1, discount_factor=0.9, exploration_rate=1.0, min_exploration_rate=0.01, exploration_decay=0.995):
        self.grid = grid
        self.rows, self.cols = grid.shape
        self.q_table_1 = np.zeros((self.rows, self.cols, 4))
...
def main():
    ...
    start_state_E = (0, 0)
    agent_clean = DoubleQLearningAgentClean(E)
    ...
    agent_stations = DoubleQLearningAgentStations(G)
